...
export default defineComponent({
	...
	setup() {
		...
		const onSignIn = async () => {
			...
			const { data } = await login(state.loginFrom);
			...
			// 用户信息模拟数据
			const userInfos = {
				username: state.loginFrom.username,
		...
		const signInSuccess = () => {
			...
			const finalPath = route.query?.redirect as string;
			const param = route.query?.params as string;
			if (route.query?.redirect) {
				router.push({
					path: finalPath,
					query: Object.keys(param).length > 0 ? JSON.parse(param) : '',
				});
			} else {
				router.push('/');
			}
